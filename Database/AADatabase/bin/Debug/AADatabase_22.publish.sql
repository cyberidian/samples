...
GO
PRINT N'Dropping unnamed constraint on [content].[GameCategories]...';


GO
ALTER TABLE [content].[GameCategories] DROP CONSTRAINT [DF__GameCateg__Activ__32E0915F];


GO
PRINT N'Dropping [content].[FK_GameCategories_Games]...';


GO
ALTER TABLE [content].[GameCategories] DROP CONSTRAINT [FK_GameCategories_Games];


GO
PRINT N'Starting rebuilding table [content].[GameCategories]...';
...
CREATE TABLE [content].[tmp_ms_xx_GameCategories] (
    [CategoryId] INT NOT NULL,
    [GameId]     INT NOT NULL,
    [Active]     BIT DEFAULT 1 NOT NULL,
    CONSTRAINT [tmp_ms_xx_constraint_PK_GameCategories1] PRIMARY KEY CLUSTERED ([CategoryId] ASC, [GameId] ASC)
);

IF EXISTS (SELECT TOP 1 1 
           FROM   [content].[GameCategories])
    BEGIN
        INSERT INTO [content].[tmp_ms_xx_GameCategories] ([CategoryId], [GameId], [Active])
        SELECT   [CategoryId],
                 [GameId],
                 [Active]
        FROM     [content].[GameCategories]
        ORDER BY [CategoryId] ASC, [GameId] ASC;
    END

DROP TABLE [content].[GameCategories];

EXECUTE sp_rename N'[content].[tmp_ms_xx_GameCategories]', N'GameCategories';

EXECUTE sp_rename N'[content].[tmp_ms_xx_constraint_PK_GameCategories1]', N'PK_GameCategories', N'OBJECT';
...
GO
ALTER TABLE [content].[GameCategories] WITH NOCHECK
...
GO
ALTER TABLE [content].[GameCategories] WITH CHECK CHECK CONSTRAINT [FK_GameCategories_Games];
